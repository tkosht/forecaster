...
def create_linears(dim_emb, kq, ws):
    return nn.Seuquential(
        nn.Linear(dim_emb * ws, dim_emb), nn.ReLU(), nn.Linear(dim_emb, kq)
    )
...
class Cyclic(M):
    ...
    def forward(
        self, ti: torch.Tensor, tc: torch.Tensor, kn: torch.Tensor, tg: torch.Tensor
    ):
        # embedding
        ...
        def reshape(tsr: torch.Tensor):
            return tsr.view(-1, self.args.dim_emb * self.args.ws)
        ...
        # adjusting the shape
        kq = self.args.k * self.args.n_quantiles
        t = ti[:, -1, :].repeat(1, kq)
        w = w.view(-1, self.args.k, 1).repeat(1, 1, 7).view(-1, kq)
        o = o.view(-1, self.args.k, 1).repeat(1, 1, 7).view(-1, kq)

        # calculate theta (rad)
        th = 2 * numpy.pi * w * t + o
        y = a * torch.cos(th) + b * torch.sin(th)
        y = y.view(-1, self.args.k, self.args.n_quantiles)
        y = y.sum(dim=1)
        ...
        for idx in range(epochs):
            ...
            def closure():
                ...
                if idx % self.params.log_intervals == 0 and idx > 0:
                    ...
                    def write_log2tb(preds, pred_type="train") -> None:
...
class DatasetToy(object):
    def __init__(self, B: int, W: int, Dout: int):
        self.B = B
        self.W = W
        self.Dout = Dout
    ...
    def create(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        self.ti = ti.float()
        self.tc = torch.rand((B, 1, 1)).repeat(1, W, 3)  # shape: (B, W, Dtc)
        self.kn = torch.rand((B, W, 4))  # shape: (B, W, Dkn)
...
if __name__ == "__main__":
    ...
    toydataset = DatasetToy(B, W, Dout)
    ...
    # train
    trainer.do_train(dataset, epochs=10 * 1000)
